...
#        gsub(/\{2_months_ago\}/, 2.month.ago.strftime('%Y-%m'))
  ...
  def token_replaced_sql_string(sql_string, filter_prefs)
    ...
    if filter_prefs.present?
      filter_prefs.each do |fp|
        ...
        ########## connection type ####################
          ...
          query_array << "#{fp.filter_param.column} = '#{sql_quoted(fp.selected)}'" if fp.values.present?

        ########## check boxes ####################
        elsif filter_param_type == FilterParam::CHECK_BOXES

          if fp.filter_enabled? && fp.values.present?
            check_box_array = []
  ...
  def filtered_results(filter_prefs, options = {})
    self.query_result(filter_prefs, options)
  end
