class Report < ActiveRecord::Base
  ...
  has_many :filter_params, :as => :optionable, :dependent => :destroy
...
#    raise 'columns must not be blank' if columns.blank?
  ...
  def self.check_and_convert_to_utc_ms(date_col, val)
    Rails.logger.warn "Checking and Converting to UTC"
    if val.is_a?(Time)
      val.to_i * 1000
      ...
      begin
      #Rails.logger.warn "Converting using DateTime " + date_col + " val:" + val.to_s + " type: " + val
      DateTime.strptime(val, '%Y-%m').to_i * 1000
      rescue
      val
      end
      #Rails.logger.warn "Converted!"
