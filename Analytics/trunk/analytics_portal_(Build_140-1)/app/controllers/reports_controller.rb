class ReportsController < ApplicationController
  ...
  # GET /reports/1/edit
  def edit
    @report.filter_params.each do |filter_param|
      if @report.filter_prefs.select {
          |filter_pref| filter_pref.filter_param == filter_param
      }.blank?
        @report.filter_prefs.build(:filter_param_id => filter_param.id)
      end
    end
  end
  ...
  def update
    ...
    respond_to do |format|
      begin
        h = params[:report]['filter_prefs_attributes']
        h.each do |k,v|
          if not v['values'].nil? then
            if not v['values'].respond_to?(:keys) then
              v['values'] = {:value => v['values']}
            end
          end
          begin
            if v['_destroy'].to_i == 1 then
              _id = v['id'].to_i
              _filter_prefs = FilterPref.where(:id => _id)
              _filter_prefs.each {|fp| fp.destroy } # Rails was not handling this so now it is...
              _keys << k
            end
          rescue
          end
      ...
      begin
        _keys.each { |x| params[:report]['filter_prefs_attributes'].delete(x) } # Rails cries unless this is done...
      rescue => e
        puts e.to_s
      end
